...
def maybe_sync_slack_token(iface, last_sync_check: float) -> float:
    """Periodically re-read the Slack bot token from disk into the monitor.

    Runs at most every GH_TOKEN_SYNC_INTERVAL seconds. When
    ``~/.agent_settings.json`` or ``/dev/shm/mcp-token`` holds a newer token
    than the long-lived ``SlackInterface`` instance, updates the shared in-memory
    holder so reactions and reads use the rotated credential.

    Skipped for non-Slack channels. Returns the timestamp of this check.
    """
    now = time.time()
    if now - last_sync_check < GH_TOKEN_SYNC_INTERVAL:
        return last_sync_check

    if resolve_messaging_channel() != "slack":
        return now

    try:
        from clients.token_resolver import TokenProvider, mcp_to_resolved

        refresh_config("agent_config")
        cached = load_agent_config().get("bot_token")
        mcp_resolved = mcp_to_resolved(TokenProvider.SLACK)
        mcp_bot = None
        if mcp_resolved and not mcp_resolved.is_expired:
            mcp_bot = mcp_resolved.bot_token
        fresh = mcp_bot or cached

        current = iface.tokens.bot_token if getattr(iface, "tokens", None) else None
        if fresh and fresh != current:
            iface._apply_bot_token(fresh)
            print("🔄 Slack bot token synced from disk", flush=True)
    except Exception as e:
        print(f"⚠️ Slack token sync failed: {e}", file=sys.stderr)
...
""",
        ...
        try:
            while True:
                ...
                last_slack_sync_check = maybe_sync_slack_token(
                    iface, last_slack_sync_check
                )
