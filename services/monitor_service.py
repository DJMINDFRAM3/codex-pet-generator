...
def start_live_progress(iface, pending_messages: list) -> bool:
    ...
    try:
        if not is_feature_enabled(LIVE_PROGRESS_FEATURE_FLAG, default=False):
            return False
        # Posting what we can't edit or delete would strand it in the channel.
        if not getattr(iface, "supports_message_edit", False):
            return False
        ...
        thread_ts = next(
            (
                m.get("thread_ts") or m.get("timestamp") or m.get("ts")
        ...
        live_progress.arm(channel=iface.default_channel, thread_ts=thread_ts)
        return True
...
def end_live_progress(iface) -> None:
    """Remove the progress message once the run is over.

    The agent normally deletes it when it posts the answer (see ``cmd_say``).
    This catches the runs that never get there — a crash, or a budget hand-off.
    ...
    try:
        ...
        try:
            if state.get("ts"):
                iface.delete_message(state["ts"], channel=state.get("channel"))
        finally:
            # Forget it either way, or the next run inherits a dead message id.
